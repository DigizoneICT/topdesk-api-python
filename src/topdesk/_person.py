...
class person:
    ...
    def get(self, id):
        return self.utils.handle_topdesk_response(
                self.utils.request_topdesk("/tas/api/persons/id/{}".format(id)))

    def get_list(self, archived=False, page_size=100, query=None):
        return self.utils.handle_topdesk_response(
                self.utils.request_topdesk("/tas/api/persons/", archived, page_size, query))

    def create(self, **kwargs):
        return self.utils.handle_topdesk_response(
                self.utils.post_to_topdesk(
                    "/tas/api/persons", (self.utils.add_id_jsonbody(**kwargs))))

    def update(self, person, **kwargs):
        return self.utils.handle_topdesk_response(
                self.utils.put_to_topdesk(
                    "/tas/api/persons/id/{}".format(person), self.utils.add_id_jsonbody(**kwargs)))
    ...
    def archive(self, person_id, reason_id=None):
        ...
        return self.utils.handle_topdesk_response(
                self.utils.put_to_topdesk(
                    "/tas/api/persons/id/{}/archive".format(person_id), param))

    def unarchive(self, person_id):
        return self.utils.handle_topdesk_response(
                self.utils.put_to_topdesk(
                    "/tas/api/persons/id/{}/unarchive".format(person_id), None))
        ...
        def get(self, person_id):
            return self.utils.handle_topdesk_response(
                    self.utils.request_topdesk(
                        "/tas/api/persons/id/{}/contract".format(person_id)))
    ...
    class _private_details:
        ...
        def get(self, person_id):
            return self.utils.handle_topdesk_response(
                    self.utils.request_topdesk(
                        "/tas/api/persons/id/{}/privateDetails".format(person_id)))
